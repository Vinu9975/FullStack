...
export default function BasicTable2(props) {
  ...
  const [searchItem, setSearchItem] = useState(""); //for serching textbox
  ...
  useEffect(() => {
    ...
    setData(datasearch.slice(iteamOffset, endOffset));
    setPageCount(Math.ceil(datasearch.length / 10))
  }, [iteamOffset,data]);
  
  var datasearch = tickets.filter((ticket) => {
    return Object.keys(ticket).some((key) => {
      return ingnore.includes(key)?false: ticket[key]
        .toString()
        .toLowerCase()
  ...
  const updateticket = (_id) => {
    if (nweDesc === desc || nweDesc==="") {
      alert("Ticket Not Updated");
    } else {
      dispatch(updateTicket(_id, Day, nweDesc));
      setNewdesc("")
    }
    setOpen(false);
  ...
  return (
    <>
        ...
        <Table sx={{ minWidth: 650 }} aria-label="simple table" className={classes.table}>
          <TableHead sx={{ backgroundColor: "#4B7BE5" }}>
            <TableRow>
              ...
              <TableHeaderCell align="center">Created Date</TableHeaderCell>
              <TableHeaderCell align="center">Updated Date</TableHeaderCell>
              <TableHeaderCell align="center">Deleted Date</TableHeaderCell>
          ...
          <TableBody>
            {data?.map((ticket, index) => (
                <TableRow
                  key={ticket._id}
                  style={{
                    backgroundColor: ticket.isDelete ? "#a3cfbb" : "#DBDBDB",
                  }}
                >
                  ...
                  <TableCell align="center">
                    <button
                      disabled={ticket.isDelete}
                      type="button"
                      className="btn btn-info"
                      onClick={() => {
                        handleClickOpen(ticket._id);
                      }}
                      style={{
                        display: ticket.Email === email ? "block" : "none",
                      }}
                    ><EditOutlinedIcon/>
                      Update
                    </button>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <TextField hidden defaultValue={_id} />
